...
$querycpu = mysqli_query($connect,"SELECT UNIX_TIMESTAMP(date)as date, max(100-idle)as used FROM cpurep where dbid in(SELECT dbuid from configureddb where server ='$server') group by date(date) order by date desc limit 30;");
