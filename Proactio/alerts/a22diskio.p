 ...
 *******************************************************************************
 *
 * Disk IO status % free alert
 * alert id  = 22
 * initial Threshold = 90%
 *
 *
 * alert generated when disk io is greater than threshold value
 * uses "scripts/diskio" script 
 * Parameters :
 *  --dbid : unique id assigned to that database - Type : CHARACTER
 *  --serv :servername
 *  --dThreshold : Threshold value
 *
 * Known Bugs & Issues:
 *
 *
...
DEFINE VARIABLE iAlertid        AS INTEGER    NO-UNDO INITIAL 22.  
DEFINE VARIABLE dPrevThreshold  AS DECIMAL    NO-UNDO.
DEFINE VARIABLE cRes            AS CHARACTER  NO-UNDO.
DEFINE VARIABLE diostat           AS DECIMAL    NO-UNDO.
...
MESSAGE getdate() "Running disk io script for server : " serv.
INPUT THROUGH sh value(lcScriptPath + "diskio.sh") value(serv).
IMPORT unformatted cRes.
/*diostat = DECIMAL(cRes).*/
diostat = DECIMAL(ENTRY(1,cRes,".")).
MESSAGE getdate() "Calculated Disk IO % :" string(diostat).
if diostat >= dThreshold then
DO:    
        ...
        DO:
           ...
           if ( diostat <> dPrevThreshold ) then
           DO:           
            lcExecute = 'UPDATE alerts SET info = "' + string(diostat) + '%.", date = NOW()
