 ...
 *******************************************************************************
 *
 * Physical Memory % free alert
 * alert id  = 23
 * initial Threshold = 10%
 *
 * alert generated when available Physical Memory is less than threshold value
 * uses "scripts/physicalmem" script
 * Parameters :
...
DEFINE VARIABLE iAlertid        AS INTEGER    NO-UNDO INITIAL 23.
...
MESSAGE getdate() "Running physical memory script for server : " serv.
INPUT THROUGH sh value(lcScriptPath + "physicalmem.sh") value(serv).
IMPORT unformatted cRes.
/*dFree = DECIMAL(cRes).*/
dFree = DECIMAL(ENTRY(1,cRes,".")).
MESSAGE getdate() "Calculated Physical Memory free % :" string(dFree).
if dFree <= dThreshold then
DO:                                       
        lcExecute='SELECT info , alertid from alerts where  desc_id = ' + string(iAlertid) + ' AND dbid ="' + dbid + '" AND alert_read = 0  \'|sed \'s/\t/,/g'.        lcQueryRes = mysqlSelect (input lcExecute).        
        if ( lcQueryRes = "" ) THEN
        DO:
           /* Run insert*/
           lcExecute = 'INSERT INTO alerts (desc_id,dbid,date,alert_read,info) VALUES (' 
                                                                + string(iAlertid) + ',"'
                                                                + dbid + '", NOW(),0,"'
                                                                + string(dFree) + '% Physical Memory space Left.");'.
        ...
        DO:
           ...
           DO:           
            lcExecute = 'UPDATE alerts SET info = "' + string(dFree) + '% Physical Memory space Left." ,date = NOW()
